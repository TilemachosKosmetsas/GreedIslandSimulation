...
# If a pkl file does not exist already then generate one
if os.path.exists("map_grid.pkl"):
    with open("map_grid.pkl", "rb") as f:
        map_grid = pickle.load(f)
else:
    # Import the map generation module and run the functions to create a new map.
    import map  # Ensure this file contains the map creation functions.

    map.create_island()
    map.create_irregular_lake()
    map.create_walls()
    map.create_forest_clusters()
    map_grid = map.map_grid  # Assuming map.py exposes the generated map as map_grid

    # Save the new map to a pickle file for future use.
    with open("map_grid.pkl", "wb") as f:
